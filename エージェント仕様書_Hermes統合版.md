...
> Hermesにはもう一つの機能領域として、企画書7〜8章で定義した「クローズドコミュニティでの心理的安全性を保ったコミュニケーション連携・相乗効果検出」がある。個人のリアルタイムな感情やモチベーションを追跡・スコア化する監視的アプローチは「非監視の原則」に反するため明確に排除し、全体・チーム単位の匿名集計による「温度感インデックス」および「自発的ケア要請」として位置づける。これらは引き続きMVPのフェーズ2以降で実装する対象であり、本書の対象外とする。ただし、両機能は同じ「Hermes」という一つの人格・設計原則を共有するため、次節でその原則を明記する。
...
- **非監視原則の徹底（個人感情監視・自動評価の排除）**：個人の感情やモチベーションをリアルタイムに監視・スコア化することは「監視されている感覚」を招きコミュニティの自律性を損なうため明確に排除する。健康度の把握は「チーム・コミュニティ全体の匿名温度感（Temperature Index）」に限定し、個人へのサポートは本人の自発的シグナル（Self-initiated Care Routing）を起点とする。
...
#### 2.4.6 匿名化レベル（Lv1）における自然言語処理（NLP）ベースの抽出

**ご提案の趣旨**：トリプル抽出をLLMではなく自然言語処理（形態素解析・固有表現認識・係り受け解析）で行うことで、AIエージェントが必要とする「メタ情報」（エンティティの種別や関係の種類）だけを検知させ、生のテキストそのものを外部LLMに渡さずに済ませる、という方式として仕様化する。

**背景・課題**：現行の `triple_extract` はLLM（Gemini API）に生のテキストを送信して主語・述語・目的語を抽出する設計になっている。しかし、自己開示レベル（企画書5章）のLv1（AI分析のみ・匿名）は、本来「個々の発言内容をそのまま渡さない」という前提のはずであり、LLMに生テキストを送ること自体がこの前提とやや矛盾する。

**提案する構成**：

- Lv1のテキストに限り、外部LLM APIを介さず、**ローカルで完結する自然言語処理パイプライン**でトリプル相当の構造を抽出する。
  - 形態素解析・係り受け解析：GiNZA（Megagon Labs製、spaCy＋Sudachiベースの日本語処理OSSライブラリ）等、ローカル実行可能なライブラリを用いる
  - 固有表現認識（NER）：人物・組織・日付・数値等のエンティティ種別を検出する（GiNZAは拡張固有表現階層に基づくNERモデルを内蔵）
  - 係り受け解析の結果から、主語にあたる文節と目的語にあたる文節、その間の述語（動詞句）を機械的に抽出し、トリプル候補とする
- Hermes（分析エンジン）に渡るのは、**エンティティの種別と関係の種類（メタ情報）のみ**であり、元の生テキストの語彙・言い回しそのものは分析対象に含めない、という運用を徹底する。

**Lv2／Lv3との使い分け（ハイブリッド方針）**：

| 開示レベル | 抽出方式 |
|---|---|
| Lv1（匿名） | NLPベース抽出のみ。外部LLMには一切テキストを送らない |
| Lv2／Lv3（本人が共有・公開に同意済み） | 従来通りLLM（Gemini）ベースの抽出を用い、より高精度な意味理解を活用する |

**トレードオフ**：

- NLPベースの抽出は、LLMほど柔軟な意味理解ができないため、複雑な文や暗黙の関係性（比喩表現、複数文にまたがる文脈依存の関係等）の抽出精度は相対的に下がる。
- 一方で、決定論的でblack-boxではないため、「何を根拠に何を抽出したか」を後から検証しやすく、匿名化の実施内容を監査・説明しやすいという利点がある。
- Lv1の文脈では、多少抽出精度が粗くなっても「生テキストを外部に一切出さない」ことを優先すべきであり、精度とプライバシーはこの範囲ではトレードオフの関係にあると整理する。

**データモデルへの反映**：`Triple.source_agent` に `"nlp_extractor"` という値を追加し、LLMベース抽出（`"triple_extractor"`）とNLPベース抽出を区別・追跡できるようにする。

**MVPでの扱い**：NLPベースの抽出パイプラインの構築（ライブラリ選定・精度検証）は一定の実装コストを伴うため、MVP時点ではLv1自体を簡易実装にとどめている方針（MVP仕様書3.2節）と整合させ、本節の内容はフェーズ2以降でLv1を本格実装する際の技術方針として位置づける。
...
### 7.3 エージェントによる発見のアニメーション表現

Hermesの探索によって新しいノードやリンクが生成された際、それを静的な差分としてではなく、コミュニティが「生きて成長している」と感じられる視覚効果として表現する。

#### トリガー条件

| イベント | アニメーション種別 |
|---|---|
| Hermesの提案がオーナーに承認され、新規Entityが追加される | ノード誕生アニメーション（強調演出） |
| Hermesの提案がオーナーに承認され、新規Triple／リンクが追加される | リンク誕生アニメーション（強調演出） |
| ユーザーが手動でコンテキストを追加し、新規Entity/Linkが即時抽出される | ノード/リンク誕生アニメーション（控えめな演出） |
| 日時解決エージェントによる `info_date` の自動適用など、既存ノードの属性更新のみ | 誕生アニメーションは行わず、ごく短い「更新パルス」のみ |
| 矛盾修正エージェントによる統合・修正の適用 | 対象ノードに一瞬の「修正パルス」を表示（誕生ではなく変化を示す控えめな演出） |

#### ノード誕生アニメーション

1. スケール0→100%へのイーズアウトのスケールアニメーション（300〜500ms）
2. 同時に、ノードの色に応じたリング（波紋）を1〜2回、外側に拡大させながらフェードアウトさせる「パルスリング」演出（合計800ms程度）
3. Hermes（AI）による追加の場合のみ、ノードの近くに小さなスパークル/AIバッジアイコンを1〜2秒間表示し、その後フェードアウトする（ユーザー追加とAI発見を視覚的に区別する）
4. アニメーション終了後は通常のノード表示に戻し、恒常的な演出は残さない（読みやすさを優先する第8章の原則を踏襲）

#### リンク誕生アニメーション

1. リンクをsource→target方向へストロークが伸びていくように描画する「ドローイン」演出（400〜600ms）
2. react-force-graph-2dの `linkDirectionalParticles` 機能を用いて、誕生直後のリンクにのみ数秒間パーティクル（光の粒）を流し、関係が生まれたことを視覚的に伝える。数秒後は通常状態（パーティクルなし、または常時ごく少数）に戻す
3. リンクのラベル（`predicate`、2.4節参照）も、リンクの描画完了と同時にフェードインで表示する

#### 複数件同時発生時の制御

Hermesの日次バッチで一度に多数の変更が承認されると、全てを同時にアニメーションさせると画面が煩雑になり、可読性改善（H8）の方針と矛盾する。そのため：

- 新規ノード・リンクは同時に最大3〜5件程度までとし、それ以上は100〜200ms程度の間隔で順番に「発見シーケンス」として再生する
- 一度に10件を超える変更がある場合は、個別アニメーションの代わりに「🔔 昨日から12件の新しい発見があります」というサマリーバナーを表示し、クリックすると変更点一覧（またはタイムラプス再生）を表示する

#### 表示タイミングとスコープ

- Hermesはデイリーバッチで動作するため、オーナー・メンバーがオンラインの間にリアルタイムでアニメーションが走ることは基本的に想定しない
- 代わりに、ユーザーがダッシュボードを開いた際、`GraphPreference.last_viewed_at` 以降に承認・適用された変更を「新規」とみなし、ダッシュボード読み込み時に発見アニメーションとして再生する
- 再生後、`last_viewed_at` を現在時刻に更新し、次回訪問時には同じ変更を再度アニメーションさせない

#### 個人への通知との連携

`affects_person_id` が自分自身であるユーザーがダッシュボードを開いた場合、自分に関するノードの誕生・更新アニメーションには通常と異なる強調色（例: 金色のリング）を用い、「自分に関する新しい発見」であることが一目でわかるようにする。これはHermesの「対象者への透明性」原則（1.2節）を、視覚的な形で実現するものである。

#### アクセシビリティ・設定

- OSやブラウザの `prefers-reduced-motion` 設定を尊重し、有効な場合はスケール・パルス等の動きを伴う演出を無効化し、色や枠線の変化など静的な強調表示のみに切り替える
- 表示設定パネル（グラフスタイルのカスタマイズ機能）に「発見アニメーションのON/OFF」トグルを追加し、演出を好まない利用者は完全に無効化できるようにする

#### データモデルへの追加

| テーブル | 追加カラム | 説明 |
|---|---|---|
| `GraphPreference` | `last_viewed_at` | `DateTime`。このユーザーがダッシュボードを最後に開いた日時。以降の変更を「新規」として扱う基準になる |
| `GraphPreference` | `discovery_animation_enabled` | `Boolean`（デフォルト `true`）。発見アニメーションのON/OFF設定 |
| `Entity` | `created_by_agent` | `String(50)`（nullable）。Hermesのどのサブエージェントが作成したか。`NULL`の場合はユーザー手動追加 |

#### 将来の拡張（フェーズ2以降）

- **成長のタイムラプス再生**：任意の期間（例: 過去1週間・1ヶ月）のノード・リンクの追加を時系列順にアニメーション再生する機能。コミュニティの成長を俯瞰的に振り返る体験として提供する
- **相乗効果候補の特別演出**：単なる新規ノード追加ではなく、相乗効果候補として検出された組み合わせには、通常の発見アニメーションよりも一段階強い演出（例: 2つのノードを結ぶ光の弧）を与え、重要な発見であることを強調する
- **3D空間可視化（3D Constellation）の延期と検証順序**：視覚的演出効果の高い3D表現は、MVPにおける2Dグラフ（react-force-graph-2d）での実用性・可読性の仮説検証が完了するまで延期し、フェーズ2以降の拡張として位置づける
...
| **非監視原則の遵守** | 個人の感情・モチベーションのリアルタイム監視やスコア化は禁止。扱う場合は「チーム全体の匿名集計（温度感インデックス）」および「本人の自発的要請（Self-initiated Care）」に限定する |
| **貢献度評価の合意形成** | AIによる一方的な個人貢献度スコアリングは行わず、コミュニティ全体の合意形成とルール策定を前提とする |
...
コミュニティの合意形成状況およびフィードバックを踏まえ、将来機能の開発優先度を以下のように整理する。

### 9.1 優先度【高】：相乗効果とリソースのプーリング
- **リソース・相乗効果プーリング（Pooling Engine）** — 各メンバーのスキル、関心テーマ、余剰リソース（稼働時間や設備等）をプールし、潜在的な協業機会や相乗効果（Synergy）を検知してマッチング提案を行う。
- **Agent 4: レポート生成エージェント** — 週次でネットワークの変化サマリーやプーリング状況を自動生成。
- **Agent 5: マッチングエージェント** — potential リンクやプーリングされたリソースの中から特に相乗効果が高そうなペアをオーナーに推薦。

### 9.2 優先度【中】：心理的安全性を保ったコミュニケーション連携
- **匿名温度感インデックス（Temperature Index）** — 個人の感情を追跡・スコア化するのではなく、チャンネルやチーム全体の活動リズム・トーンを統計的に匿名集計し、コミュニティの活性度や疲弊傾向を可視化する。
- **自発的ケア要請ルーティング（Self-initiated Care Routing）** — メンバーが自発的にサポートを求められる専用窓口・トリガーを設け、適切なメンターや相談先エージェントへ安全に接続する。
...
### 9.3 優先度【低／合意形成後】：評価・高度表現
- **貢献度スコアリング（Contribution Scoring）** — コミュニティ内での評価基準・配分比率に関する十分な合意形成が行われた段階で検討。AIによる一方的な自動評価ではなく、透明性ある算定支援ツールとして位置づける。
- **3D空間可視化（3D Constellation）** — MVPでの2Dグラフによる実用性・操作性の仮説検証完了後に、空間コンピューティングやWebXR/3D演出への拡張として検討。
